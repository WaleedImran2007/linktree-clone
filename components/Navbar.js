      ...
      <nav className='bg-white w-[90vw] fixed top-10 right-[5vw] rounded-full px-4 py-3 flex justify-between items-center shadow-lg'>
        ...
        <div className='block px-6 lg:hidden'>
          <button onClick={() => setmenuOpen(!menuOpen)}>
            <svg
              className="w-6 h-6 text-black cursor-pointer hover:text-gray-500"
              fill="none"
              stroke="currentColor"
              strokeWidth={2}
              viewBox="0 0 24 24"
        ...
        <div className='lg:flex gap-2 hidden'>
          <button className=' bg-gray-100 px-5 py-1 text-lg rounded-lg font-bold hover:bg-gray-200 cursor-pointer '>Log in</button>
          <button className=' bg-slate-900 text-white px-6 py-4 rounded-full font-[600] hover:bg-slate-800 cursor-pointer '><span>Sign up</span><span> free</span></button>
        </div>

        {menuOpen && (
          <ul className='flex flex-col gap-7 text-[15px] font-[400] text-[rgb(30, 35, 48)] absolute top-14 left-0 w-full z-50 items-center bg-white rounded-2xl py-4 lg:hidden'>
            <li className='cursor-pointer hover:font-bold'><Link onClick={() => { setmenuOpen(false) }} href={"/"}>Templates</Link></li>
            <li className='cursor-pointer hover:font-bold'><Link onClick={() => { setmenuOpen(false) }} href={"/"}>Marketplace</Link></li>
            <li className='cursor-pointer hover:font-bold'><Link onClick={() => { setmenuOpen(false) }} href={"/generate"}>Generate</Link></li>
            <li className='cursor-pointer hover:font-bold'><Link onClick={() => { setmenuOpen(false) }} href={"/"}>Learn</Link></li>
            <li className='cursor-pointer hover:font-bold'><Link onClick={() => { setmenuOpen(false) }} href={"/"}>Pricing</Link></li>
            <div className='flex gap-4'>
              <button onClick={() => { setmenuOpen(false) }} className=' bg-gray-100 px-5 py-1 text-lg rounded-lg font-bold hover:bg-gray-200 cursor-pointer '>Log in</button>
              <button onClick={() => { setmenuOpen(false) }} className=' bg-slate-900 text-white px-6 py-4 rounded-full font-[600] hover:bg-slate-800 cursor-pointer '><span>Sign up</span><span> free</span></button>
            </div>
          </ul>
        )}
